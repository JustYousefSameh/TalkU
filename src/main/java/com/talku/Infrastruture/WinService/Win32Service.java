...
public abstract class Win32Service {
    ...
    /**
     * Service SID information. Not found in JNA so I created it (works like
     * SERVICE_DESCRIPTION) see Winsvc.h for more information
     * 
     * @param dwServiceSidType is the service security identifier type
    ...
    public static class SERVICE_SID_INFO extends ChangeServiceConfig2Info {
     ...
     */
    public boolean install(String displayName, String description, String[] dependencies, String account,
            String password) {
        ...
        try {
            ...
            System.out.println("Failed to close classloader: " + e.getMessage());
        }
        try {
    ...
    public boolean install(String displayName, String description, String[] dependencies, String account,
        ...
        // For wireguard to work, SID must to be set to UNRESTRICTED
        SERVICE_SID_INFO sid = new SERVICE_SID_INFO();
        sid.dwServiceSidType = 0x00000001; // SERVICE_SID_TYPE_UNRESTRICTED
        ...
        if (serviceManager != null) {
            SC_HANDLE service = advapi32.CreateService(serviceManager, serviceName, displayName,
                    ...
                    WinNT.SERVICE_ERROR_NORMAL, command, null, null, dep.toString(), account, password);

            if (service != null) {
                success = advapi32.ChangeServiceConfig2(service, Winsvc.SERVICE_CONFIG_SERVICE_SID_INFO, sid);
    ...
    public boolean start() {
        ...
        if (serviceManager != null) {
            ...
            advapi32.QueryServiceStatus(service, serviceStatus);
            while (success == true && (System.currentTimeMillis() - startTime < timeout
            && serviceStatus.dwCurrentState != Winsvc.SERVICE_RUNNING)) {
                ...
                advapi32.QueryServiceStatus(service, serviceStatus);
                if (serviceStatus.dwCurrentState == Winsvc.SERVICE_STOPPED) {
                    System.out.println("Service " + serviceName + " failed to start");
                    success = false;
                    break;
                }
            }
            if (serviceStatus.dwCurrentState == Winsvc.SERVICE_RUNNING && System.currentTimeMillis() - startTime < timeout) {
                success = true;
            }  else success = false;
            advapi32.CloseServiceHandle(service);
            advapi32.CloseServiceHandle(serviceManager);
